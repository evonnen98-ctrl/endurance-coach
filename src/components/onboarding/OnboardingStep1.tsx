  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all ${
        selected ? 'border-transparent' : 'border-gray-200 bg-white text-gray-900 hover:border-gray-400'
      }`}
      style={selected ? { backgroundColor: 'var(--volt)', color: 'var(--ink)' } : undefined}
    >
      <span className="text-xl flex-shrink-0">{emoji}</span>
      <div>
        <div className="font-semibold text-sm leading-tight">{label}</div>
        <div className={`text-xs mt-0.5 leading-tight ${selected ? 'text-gray-600' : 'text-gray-500'}`}>{desc}</div>
      </div>
    </button>
  )
}
    ...
    <div className="px-5 pb-8">
      ...
      <div className="pt-6 mb-7">
        ...
        <div className="flex gap-1.5 mt-2">
          {[1,2,3,4].map(i => (
            <div key={i} className={`flex-1 h-0.5 rounded-full ${i <= 1 ? '' : 'bg-gray-200'}`} style={i <= 1 ? { backgroundColor: 'var(--volt)' } : undefined} />
          ))}
        </div>
      </div>
